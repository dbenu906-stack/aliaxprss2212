...
// Get all active viva banners
export async function GET() {
  try {
    const connection = await pool.getConnection();
    const [banners] = await connection.execute(
      'SELECT * FROM viva_banners WHERE is_active = TRUE ORDER BY created_at DESC'
    );
    connection.release();

    return NextResponse.json({ data: banners }, { status: 200 });
  } catch (error) {
    console.error('Error fetching viva banners:', error);
...
// Create new viva banner (admin only)
export async function POST(req: NextRequest) {
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO viva_banners (title, image_url, subtitle, button_text, button_link, background_color)
       VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    console.error('Error creating viva banner:', error);
    return NextResponse.json(
